...
class GlobalIndexScanBuilder(ABC):
    ...
    def parallel_scan(
        ...
        """
        Scan global indexes in parallel across multiple ranges.
        
        Args:
            ranges: List of row ranges to scan
            builder: The GlobalIndexScanBuilder instance
            filter_predicate: Optional filter predicate
            vector_search: Optional vector search query
            thread_num: Number of threads to use
            
        Returns:
            Combined GlobalIndexResult or None if no results
        """
        if not ranges:
            return None
