...
class CoreOptions:
    ...
    GLOBAL_INDEX_ENABLED: ConfigOption[bool] = (
        ...
        .boolean_type()
        .default_value(False)
        .with_description("Whether to enable global index.")
    )
    ...
    GLOBAL_INDEX_THREAD_NUM: ConfigOption[int] = (
        ...
        .int_type()
        .default_value(4)
        .with_description("Number of threads for global index scanning.")
    )
    ...
    def global_index_thread_num(self, default=None):
        return self.options.get(CoreOptions.GLOBAL_INDEX_THREAD_NUM, default)
    ...
    def get_faiss_vector_options(self) -> dict:
        """Get all FAISS vector index options as a dictionary."""
        return {
            'vector.dim': self.vector_dim(),
            'vector.metric': self.vector_metric(),
            'vector.index-type': self.vector_index_type(),
            'vector.m': self.vector_m(),
            'vector.ef-construction': self.vector_ef_construction(),
            'vector.ef-search': self.vector_ef_search(),
            'vector.nlist': self.vector_nlist(),
            'vector.nprobe': self.vector_nprobe(),
            'vector.size-per-index': self.vector_size_per_index(),
            'vector.search-factor': self.vector_search_factor(),
            'vector.normalize': self.vector_normalize(),
        }
